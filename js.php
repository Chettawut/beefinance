    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            for (count = 0; count < result.supcode.length; count++) {
                ...
                let status
                if(result.status[count]=='3')
                status = 'เสร็จสิ้น'
                else if(result.status[count]=='2')
                status = 'ยื่นตามเอกสารตัวจริงแล้ว'
                else if(result.status[count]=='1')
                ...
                $('#tableCustomer').append(
                    ...
                    .supcode[
                        ...
                        count] + '"><td>' + result.s_date[count] + '</td><td>' + result.supname[count] + '</td><td>' + result.supcode[count] + '</td><td>' + (result.supcode[count]+200) + '</td><td  style="text-align:center">' + status + '</td></tr>');
            }
